...
// Outil pour générer des slugs propres
...
// Type pour une variante dans le formulaire
...
export default function ProductForm({ categories, onProductCreated }: { categories: Category[]; onProductCreated: () => void; }) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    toast.loading('Création du produit...');

    try {
      // 1. Téléverser les images vers Cloudinary
      ...
      // 2. Préparer les données pour notre API
      const form = e.currentTarget;
      const productData = {
        title: form.title.value,
        slug: form.slug.value,
        description: form.description.value,
        price_cents: Math.round(parseFloat(form.price.value) * 100),
        categoryId: form.categoryId.value,
        images: uploadedImages,
        variants: variants.map(({ name, stock }) => ({ name, stock })),
      };

      // 3. Appeler notre API d'administration
      ...
      onProductCreated(); // Ferme la modale et rafraîchit la liste
    } catch (error) {
      toast.dismiss();
